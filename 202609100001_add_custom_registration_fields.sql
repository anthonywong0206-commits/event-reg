-- Per-event configurable registration fields and per-registration custom answers.
alter table public.events add column if not exists email_required boolean not null default false;
alter table public.events add column if not exists notes_required boolean not null default false;
alter table public.events add column if not exists custom_registration_fields jsonb not null default '[]'::jsonb;
alter table public.registrations add column if not exists custom_answers jsonb not null default '{}'::jsonb;

comment on column public.events.custom_registration_fields is 'Array of admin-configured additional registration field definitions.';
comment on column public.registrations.custom_answers is 'Answers keyed by custom registration field id.';
...
begin
  ...
  insert into public.registrations(event_id, session_id, registration_no, full_name, email, phone, method, status, notes, custom_answers)
  values (p_event_id, p_session_id,
    ...
    trim(p_full_name), normalized_email, trim(p_phone), p_method, target_status, nullif(trim(p_notes),''), coalesce(p_custom_answers, '{}'::jsonb))
...
revoke all on function public.register_for_event(uuid, uuid, text, text, text, public.registration_method, text, jsonb) from public, anon, authenticated;
grant execute on function public.register_for_event(uuid, uuid, text, text, text, public.registration_method, text, jsonb) to service_role;
