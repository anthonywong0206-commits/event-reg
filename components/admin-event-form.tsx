...
import { AlertCircle, ImageUp, LoaderCircle, Save, Trash2 } from "lucide-react";
...
export function AdminEventForm({ event }: { event?: EventRecord | null }) {
  ...
  return (
    <form className="admin-event-form" onSubmit={submit}>
      ...
      <section className="admin-form-section">
        <div className="section-heading"><h2>日期、地點及名額</h2><span>系統會按名額與截止時間自動停止報名</span></div>
        <div className="form-grid">
          <label className="field"><span>開始時間 *</span><input name="start_at" type="datetime-local" required defaultValue={localInput(event?.start_at)} /></label>
          <label className="field"><span>結束時間 *</span><input name="end_at" type="datetime-local" required defaultValue={localInput(event?.end_at)} /></label>
