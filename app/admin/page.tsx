...
import { CalendarPlus, CalendarRange, CheckCircle2, ClipboardList, Clock3, Edit3, ExternalLink, ImageIcon, QrCode, UsersRound } from "lucide-react";
  ...
  return (
    <main className="admin-dashboard">
      <header className="admin-header">
        <div className="admin-shell admin-header-inner"><Link href="/admin" className="admin-brand"><span className="brand-mark"><CalendarRange /></span><span><strong>Event Register</strong><small>管理後台</small></span></Link><nav><Link href="/admin/site-settings"><ImageIcon />首頁橫額</Link><Link href="/" target="_blank"><ExternalLink />查看網站</Link><Link href="/check-in"><QrCode />現場登記</Link><SignOutButton /></nav></div>
      </header>
      <div className="admin-shell admin-content">
        <div className="admin-welcome"><div><p>歡迎回來，{profile.display_name || "管理員"}</p><h1>活動管理總覽</h1></div><div className="admin-welcome-actions"><Link className="button button-secondary button-large" href="/admin/site-settings"><ImageIcon />首頁橫額設定</Link><Link className="button button-primary button-large" href="/admin/events/new"><CalendarPlus />建立新活動</Link></div></div>
