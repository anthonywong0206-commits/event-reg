...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File)) {
      return NextResponse.json({ error: "請選擇橫額圖片" }, { status: 400 });
    }
    ...
    const path = `site/homepage/${new Date().getUTCFullYear()}/${randomUUID()}.${extension}`;
    ...
    console.error("Homepage banner upload failed", error);
    return NextResponse.json(
      { error: error instanceof Error ? `橫額圖片上載失敗：${error.message}` : "橫額圖片上載失敗" },
      { status: 500 },
    );
  }
}
