...
export default async function RegistrationSuccessPage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  const qrDataUrl = isWaitlist ? null : await createQrDataUrl(registration.qr_token);
      ...
      <main className="success-page shell">
        <section className="success-card">
          <div className="success-copy">
            ...
            <span className="success-kicker">{isWaitlist ? "候補登記完成" : "申請完成"}</span>
            <h1>{isWaitlist ? "已收到你的候補登記" : "報名成功！"}</h1>
            {isWaitlist ? (
              <p><strong>目前正選名額已滿，你已登記候補名單。</strong> 候補不代表成功參加；如沒有收到主辦單位進一步通知，則視作未能申請。</p>
            ) : registration.email ? (
            ...
            {isWaitlist ? (
              <div className="email-confirmation waitlist"><CalendarDays /><span><strong>候補登記不會獲發入場 QR Code</strong>如有名額釋出，主辦單位會另行聯絡；否則毋須再作任何操作。</span></div>
            ) : registration.email ? (
          ...
          {isWaitlist ? <div className="qr-ticket waitlist-ticket">
            <div className="ticket-top"><CalendarDays /><span>候補登記</span></div>
            <strong>{registration.registration_no}</strong>
            <span>{registration.full_name}</span>
            <p>正選名額已滿</p>
            <small>如獲安排正選名額，主辦單位會另行通知。</small>
          </div> : <div className="qr-ticket">
